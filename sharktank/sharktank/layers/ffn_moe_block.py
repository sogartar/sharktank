...
class DenseFFNMOE(ThetaLayer):
    ...
    def forward(
        ...
        top_experts_index:
            ...
            Shape of (batch_size * sequence_length, num_of_top_experts).
        ...
        router_scores = torch.full(
            [self.num_experts, num_tokens], 0, dtype=h.dtype
        ).scatter_(0, top_experts_index, expert_gate)
        ...
        routed_in = routed_in * (router_scores > 0).view(-1, 1)
        ...
        routed_out = routed_out * router_scores.view(-1, 1)

        return torch.scatter(
            torch.zeros_like(h), dim=0, index=router_indices, src=routed_out
        )
