...
class ModelConfig:
    ...
    def load(cls, config_path: PathLike, /, **kwargs) -> "ModelConfig":
        ...
        if hugging_face_repo_id is not None:
            revision = None
            if "hugging_face_revision" in config_dict:
                revision = config_dict["hugging_face_revision"]
            config_form_hf = cls.translate_hugging_face_config_into_init_kwargs(
                config_dict["hugging_face_repo_id"], revision
            )
    ...
    def asdict_for_saving(
        ...
        Here for example Path objects are made relative to the config dir converted to str."""
        config_path = config_path or self.config_path
        config_dir = None
    ...
    def translate_hugging_face_config_into_init_kwargs(
        cls, /, repo_id: str, revision: str | None = None
    ) -> dict[str, Any]:
        """Download and translate Hugging Face config into key-value pairs that can be
        used to initialize a config."""
        from huggingface_hub import model_info

        return cls.translate_hugging_face_config_properties_into_init_kwargs(
            model_info(repo_id, revision=revision).config
        )
