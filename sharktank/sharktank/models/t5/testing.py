...
def encoder_toy_config(dtype: Optional[torch.dtype] = None) -> T5Config:
        return T5Config(
            vocab_size=11,
            d_model=13,
            d_kv=7,
            ...
            context_length_padding_block_size=19,
        )


def export_encoder_toy_default_iree_test_data(output_dir: PathLike):
    ...
    export_encoder_toy_iree_test_data(
...
def export_encoder_toy_iree_test_data(
    ...
    reference_config = encoder_toy_config(reference_dtype)
    ...
    reference_model = T5Config(theta=reference_theta, config=reference_config)
    for i, (
        target_dtype,
...
def export_encoder_iree_test_data(
    ...
    target_model = T5Encoder(theta=target_dataset.root_theta, config=target_config)
    export_clip_text_model_to_iree(
        target_model,
