...
class ShardedTensorTest(unittest.TestCase):
    ...
    def runTestInPlaceUpdate(self, path_prefix: str, dump_enabled: bool):
        ...
        class Module(torch.nn.Module):
            def main(self, tensor: AnyTensor, tensor2: AnyTensor):
                tensor += 1
        ...
        module = Module()
        module.main(sharded_tensor, torch.empty([1]))
        ...
        @fxb.export_program(
            args=(sharded_tensor, torch.empty([1])),
            name="main",
            strict=False,
        )
        ...
        iree_args = prepare_iree_module_function_args(
            args=[deepcopy(sharded_tensor_snapshot), torch.empty([1])],
            devices=iree_devices,
        )
        run_iree_module_function(
            args=iree_args,
