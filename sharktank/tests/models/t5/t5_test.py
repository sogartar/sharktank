...
class T5EncoderEagerTest(TestCase):
    ...
    def testCompareToyTorchVsHuggigFace(
        self,
        target_dtype: torch.dtype,
        reference_dtype: torch.dtype,
        atol: float,
        ...
        # def assert_close(actual: dict[str, torch.Tensor], expected: dict[str, torch.Tensor]):
        #     from sharktank.utils.tree import map_nodes
        #     actual = {k: actual[k].to(dtype=v.dtype) if v is not None else None for k, v in expected.items()}
        #     torch.testing.assert_close(
